...
public sealed class WoundsVisualsSystem : EntitySystem
{
    ...
    private readonly HashSet<string> _damageGroups = ["Brute", "Burn", "Blyading"];
    ...
    public override void Initialize()
    {
        ...
        SubscribeNetworkEvent<OnWoundableLossDeleteMessage>((msg, _) => OnBodyPartLossDelete(msg));
    }

    private void InitializeEntity(EntityUid uid, WoundableVisualsComponent component, ComponentInit args)
    {
        if (!TryComp(uid, out SpriteComponent? spriteComponent)
            || component.TargetLayers is not { Count: > 0 })
            return;
    ...
    private void OnBodyPartLossDelete(OnWoundableLossDeleteMessage args)
    {
        if (!TryComp<SpriteComponent>(GetEntity(args.Body), out var spriteComponent))
            return;

        if (UpdateLostWoundableVisuals(args.Layer, spriteComponent, force: true))
            RaiseNetworkEvent(new OnWoundableLossDeleteMessage(args.Woundable, args.Body, args.Layer));
    }
    ...
    private void UpdateWoundableVisuals(EntityUid uid, BodyPartComponent bodyPart, SpriteComponent sprite)
    {
        ...
        if (layer == null
            || UpdateLostWoundableVisuals(layer.Value, sprite, severity))
            return;
    ...
    private bool UpdateLostWoundableVisuals(HumanoidVisualLayers key, SpriteComponent sprite, WoundableSeverity? severity = null, bool force = false)
    {
        sprite.LayerMapTryGet(key, out var layer);
        if (severity != WoundableSeverity.Loss && !force)
        {
    ...
    private void UpdateBleeding(EntityUid uid, HumanoidVisualLayers layer, BodyPartComponent bodyPart, SpriteComponent sprite)
    {
        if (bodyPart.PartType is BodyPartType.Foot or BodyPartType.Hand)
        {
            if (!_body.TryGetParentBodyPart(uid, out var parentUid, out _)
                || !_appearance.TryGetData<WoundsVisualizerGroupData>(parentUid.Value, WoundableVisualizerKeys.Wounds, out var parentData)
            ...
            sprite.LayerMapTryGet($"{part}Blyading", out var parentBlyadingLayer);
        ...
        else
        {
            ...
            sprite.LayerMapTryGet($"{layer}Blyading", out var blyadingLayer);
