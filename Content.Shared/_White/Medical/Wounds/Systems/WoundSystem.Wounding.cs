...
public partial class WoundSystem
{
    ...
    private void OnWoundableInit(EntityUid uid, WoundableComponent component, ComponentInit @componentInit)
    {
        // Set root to itself.
        component.RootWoundable = uid;

        // Create container for wounds.
        component.Wounds = _container.EnsureContainer<Container>(uid, WoundContainerId);
        component.Bone = _container.EnsureContainer<Container>(uid, BoneContainerId);

        InsertBoneIntoWoundable(uid, component);
    }

    private void OnWoundInserted(EntityUid woundableEntity, WoundComponent wound, EntGotInsertedIntoContainerMessage args)
    {
        if (wound.Parent == EntityUid.Invalid)
            return;

        var parentWoundable = Comp<WoundableComponent>(wound.Parent);
    ...
    /// <summary>
    /// Opens a new wound on needed woundable.
    /// </summary>
    ...
    /// <summary>
    /// Applies severity multiplier to wounds.
    /// </summary>
    /// <param name="uid">UID of the wound.</param>
    ...
    private void InsertBoneIntoWoundable(EntityUid woundable, WoundableComponent? component = null)
    {
        if (!Resolve(woundable, ref component) || _net.IsClient)
            return;
        ...
        boneComp.BoneWoundable = woundable;

        _transform.SetParent(bone, woundable);
        _container.Insert(bone, component.Bone!);
    }
    ...
    /// <summary>
    /// Destroys a entity's body part if conditions are met.
    /// </summary>
    ...
    private void DestroyWoundable(EntityUid parentWoundableEntity, EntityUid woundableEntity, WoundableComponent woundableComp)
    {
        ...
        {
            DestroyChildWoundables(woundableEntity, woundableComp);
    ...
    /// <summary>
    /// Amputates (not destroys) a entity's body part if conditions are met.
    /// </summary>
    /// <param name="parentWoundableEntity">Parent of the woundable entity. Yes.</param>
    /// <param name="woundableEntity">The entity containing the vulnerable body part</param>
    /// <param name="woundableComp">Woundable component of woundableEntity.</param>
    ...
    private bool DestroyChildWoundables(EntityUid woundableEntity, WoundableComponent? woundableComp = null)
    {
